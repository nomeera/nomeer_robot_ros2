...
from ament_index_python.packages import get_package_share_directory # Retrieves the share directory of a ROS 2 package.
...
def generate_launch_description():
    ...
    with open(sdf_file, "r") as infp:
        robot_desc = infp.read() # Reads the content of the SDF file into a string for use in other components.

    # launch gazebo
    gz_sim = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_ros_gz_sim, "launch", "gz_sim.launch.py") # Includes another launch file to start the Gazebo simulation.
        ),
        launch_arguments={
            "gz_args": PathJoinSubstitution(
                [pkg_project_description, "worlds", "tugbot_warehouse.sdf"]
            )
        }.items(),
    )
